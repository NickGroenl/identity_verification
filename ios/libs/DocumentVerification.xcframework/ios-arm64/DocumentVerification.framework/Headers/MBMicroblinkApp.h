...
@interface MDVMicroblinkApp : NSObject
...
static inline NSString * MDV_LOCALIZED_DEFAULT_STRING(NSString *key) {
    return ([NSString stringWithFormat:@"[[%@]]", key]);
}

static inline NSString * MDV_LOCALIZED_FOR_LANGUAGE(NSString *key, NSString *language) {

    NSString *overridenStringFromCustomBundle = NSLocalizedStringWithDefaultValue(key,
                                                                                  language,
                                                                                  [MDVMicroblinkApp sharedInstance].customResourcesBundle,
                                                                                  @"",
                                                                                  @"");
    ...
    return NSLocalizedStringWithDefaultValue(key,
                                             language,
                                             [MDVMicroblinkApp sharedInstance].resourcesBundle,
                                             MDV_LOCALIZED_DEFAULT_STRING(key),
                                             nil);
}

static inline NSString * MDV_LOCALIZED(NSString *key) {
    NSString *localizationFileName = [MDVMicroblinkApp sharedInstance].customLocalizationFileName;
    ...
    NSString *language = [MDVMicroblinkApp sharedInstance].language;
    NSString *localizedString = MDV_LOCALIZED_FOR_LANGUAGE(key, language);
    
#ifndef DNDEBUG
    return localizedString;
#else
    if ([localizedString isEqualToString:MDV_LOCALIZED_DEFAULT_STRING(key)]) {
        if (![language isEqualToString:@"ar-AE"] && [language hasPrefix:@"ar"]) {
            localizedString = MDV_LOCALIZED_FOR_LANGUAGE(key, @"ar-AE");
        }
        else if (![language isEqualToString:@"en"]) {
            localizedString = MDV_LOCALIZED_FOR_LANGUAGE(key, @"en");
        }
    }
    return localizedString;
#endif
